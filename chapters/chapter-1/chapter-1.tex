...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            TodoTheme {
                Scaffold(modifier = Modifier.fillMaxSize()) { innerPadding ->
                    CompteurApp(modifier = Modifier.padding(innerPadding))
                }
            }
        }
    }
...
@Composable
fun CompteurApp(modifier: Modifier = Modifier) {
    var count by remember { mutableStateOf(0) }
Column(
        modifier = modifier
            .fillMaxSize()
...
{\footnotesize
\textcolor{darkgray}{
Le cycle de vie d'un composable correspond à toutes les étapes par lesquelles il passe pendant son existence dans l'interface : \textcolor{darkgray}{\textbf{création}}, \textcolor{darkgray}{\textbf{affichage}}, \textcolor{darkgray}{\textbf{mise à jour}}, puis \textcolor{darkgray}{\textbf{disparition}}.
Contrairement aux vues traditionnelles   (\texttt{\textcolor{orange}{XML}}), Jetpack Compose ne modifie pas directement l'UI  (\texttt{\textcolor{orange}{Interface Utilisateur}}) : il recompose les éléments à partir de l'état. Cela permet une interface plus réactive, plus simple et plus performante.
}
}
...
{\footnotesize
  \textcolor{lightgray}{
    \textit{Aperçu du résultat final de l'application compteur, correspondant au code complet présenté ci-dessous.}
  }
}
\vspace{1em}

{\small \textcolor{orange}{\textbf{Le cycle de vie d'un composable:}}}
{\footnotesize
\textcolor{darkgray}{
Un composable suit un cycle de vie simple mais puissant :
\begin{itemize}
\item \textbf{Entrée dans la Composition} : Le composable est exécuté pour la première fois
\item \textbf{Recomposition} : Mise à jour en cas de modification d'état
\item \textbf{Sortie de la Composition} : Nettoyage quand le composable n'est plus affiché
\end{itemize}
Contrairement aux activités ou fragments, les recompositions sont légères et automatiques.}
  \vspace{0pt}%
 \includegraphics[width=\linewidth]{images/chapter-1/recomposition.png}
}
\end{minipage}\hfill
\begin{minipage}[t]{0.48\linewidth}
  \vspace{0pt}%
 \includegraphics[width=\linewidth]{images/chapter-1/counter-app.png}
\end{minipage}
...
\begin{minipage}[t]{0.48\linewidth}
  ...
  {\footnotesize
  \textcolor{darkgray}{
  La puissance de Jetpack Compose réside dans sa capacité à simplifier la création d'interfaces utilisateur complexes. En utilisant des fonctions Kotlin, vous pouvez créer des composants réutilisables et hautement personnalisables. L'approche déclarative permet une meilleure lisibilité du code et une maintenance simplifiée.
  }}
\end{minipage}\hfill
\begin{minipage}[t]{0.48\linewidth}
  \vspace{0pt}%
  {\footnotesize
  \textcolor{darkgray}{
  L'architecture de Compose encourage la séparation des préoccupations. Les composants d'interface utilisateur deviennent des fonctions pures qui transforment l'état en éléments visuels. Cette approche facilite les tests et améliore la maintenabilité du code, tout en offrant des performances optimales grâce à son système de recomposition intelligente.
  }}
\end{minipage}
